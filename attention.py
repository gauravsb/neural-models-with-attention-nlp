...
if __name__ == '__main__':
    attention_type = "none" if (len(sys.argv) == 1) else sys.argv[1]
    print("Running with attention type: " + attention_type)
    ...
    m = attention_model(vocab_size=len(forward_dict), hidden_dim=128, attention_type=attention_type)
    ...
    with open('model-outputs/test-attention-reverse-' + attention_type + '.txt', 'w') as the_file:
        for input_seq, gold_seq in zip(test_inputs, test_outputs):
            ...
            the_file.write(predicted_sentence + "\n")
